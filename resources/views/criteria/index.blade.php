...
<form id="criteria-form" method="POST" action="{{ route('criteria.update') }}" @if(auth()->user()->isOwner()) data-criteria-form @endif>@csrf @method('PUT')
<section class="card">
            ...
            <table>
                ...
                @foreach($criteria as $criterion)
                    <tr data-criterion-row>
                        ...
                        <td class="numeric">@if(auth()->user()->isOwner())<input class="inline-input" aria-label="Bobot kriteria {{ $criterion->name }}" type="number" step="0.01" min="0.01" max="1" name="criteria[{{ $criterion->id }}][weight]" value="{{ old('criteria.'.$criterion->id.'.weight', (float)$criterion->weight) }}" data-criterion-weight>@else<strong>{{ number_format((float) $criterion->weight * 100, 0) }}%</strong>@endif</td>
                        <td>{{ $criterion->source_description }}</td>
                        <td>@if(auth()->user()->isOwner())<label class="checkbox"><input type="checkbox" name="criteria[{{ $criterion->id }}][active]" value="1" aria-label="Aktifkan kriteria {{ $criterion->name }}" data-criterion-active @checked(old('criteria.'.$criterion->id.'.active', $criterion->active))> Aktif</label>@else<x-pill :tone="$criterion->active ? 'green' : 'gray'">{{ $criterion->active ? 'Aktif' : 'Tidak aktif' }}</x-pill>@endif</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
